...
let qqreadheaderValArr,qqreadtimeheaderValArr,YouthBody,Youthy,Youdy;qqreadtimeheaderVal,qqreadheaderVal
...
if ($.isNode()) {
    Object.keys(YouthBody).forEach((item) => {
        if (YouthBody[item]) {
          qqreadheaderValArr.push(YouthBody[item])
        }
      })
   
    Object.keys(Youdy).forEach((item) => {
        if (Youdy[item]) {
          qqreadtimeheaderValArr.push(Youdy[item])
        }
      })

for (let i = 0; i < qqreadheaderValArr.length; i++) {
    if (qqreadheaderValArr[i]) {
      qqreadheaderVal = qqreadheaderValArr[i];
      qqreadtimeheade = qqreadtimeheaderValArr[i];
      $.index = i + 1;
      console.log(`-------------------------\n\n开始【中青看点${$.index}】`)
    }
